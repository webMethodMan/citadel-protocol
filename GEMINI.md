...
* **Verification:** Uses `verify_and_gate` to weld a RIOM hash and an mTLS certificate hash into a signed 1024-byte `TDREPORT`.

### B. `proxy` (The Application Gate)
...
* **Productization Status:** Currently uses STDIO; moving toward an Axum — based SSE (Server — Sent Events) gateway.
...
* Moving from STDIO to a networked SSE transport.
